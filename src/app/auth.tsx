...
export default function AuthPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signup") {
        await signIn.email({ email, password, name, callbackURL: "/generate?tab=custom" });
      } else {
        await signIn.email({ email, password, callbackURL: "/generate?tab=custom" });
      }
    } catch (err: any) {
    ...
    <div className="max-w-md mx-auto p-4 sm:p-6">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground text-sm">
          {mode === "signin" ? "Welcome back to Orator QR" : "Start generating beautiful QR codes"}
        </p>
      </div>
      ...
      <div className="rounded-xl border p-6 space-y-4">
        ...
        <button onClick={() => signIn.social({ provider: "google", callbackURL: "/generate?tab=custom" })}
          className="w-full rounded-lg border px-4 py-2.5 text-sm font-medium hover:bg-accent transition-colors flex items-center justify-center gap-2">

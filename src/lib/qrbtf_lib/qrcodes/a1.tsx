...
function QrbtfRendererA1(props: RendererProps<QrbtfRendererA1Props>) {
        ...
        switch (typeTable[x][y]) {
          case QRPointType.POS_CENTER:
            if (props.positioning_point_type === "square") {
              points.push(
                <rect key={id++} fill={props.positioning_point_color} x={x + 0.5 - 1.5} y={y + 0.5 - 1.5} width={3} height={3} />,
              );
              points.push(
                <rect key={id++} fill="none" strokeWidth="1" stroke={props.positioning_point_color} x={x + 0.5 - 3} y={y + 0.5 - 3} width={6} height={6} />,
              );
            } else if (props.positioning_point_type === "circle") {
              points.push(
                <circle key={id++} fill={props.positioning_point_color} cx={x + 0.5} cy={y + 0.5} r={1.5} />,
              );
              points.push(
                <circle key={id++} fill="none" strokeWidth="1" stroke={props.positioning_point_color} cx={x + 0.5} cy={y + 0.5} r={3} />,
              );
              ...
              points.push(
                <circle key={id++} fill="none" strokeWidth="0.15" strokeDasharray="0.5,0.5" stroke={props.positioning_point_color} cx={x + 0.5} cy={y + 0.5} r={3} />,
              );
              for (let w = 0; w < vw.length; w++) {
                points.push(
                  <circle key={id++} fill={props.positioning_point_color} cx={x + vw[w] + 0.5} cy={y + 0.5} r={0.5} />,
                );
              }
              for (let h = 0; h < vh.length; h++) {
                points.push(
                  <circle key={id++} fill={props.positioning_point_color} cx={x + 0.5} cy={y + vh[h] + 0.5} r={0.5} />,
                );
              }
              ...
              points.push(
                <path key={id++} d={sq25} stroke={props.positioning_point_color} strokeWidth={(100 / 6) * (1 - (1 - contentPointSize) * 0.75)} fill="none"
                  transform={`translate(${x - 2.5},${y - 2.5}) scale(${6 / 100},${6 / 100})`}
                />,
              );
            }
            break;
          ...
          default:
            ...
            if (props.content_point_type === "square") {
              ...
              points.push(
                <rect opacity={props.content_point_opacity} width={size} height={size} key={id++} fill={props.content_point_color} x={x + offset} y={y + offset} />,
              );
            } else {
              const halfSize = isConstant ? contentPointSizeHalf : rand(0.3, 1) / 2;
              points.push(
                <circle opacity={props.content_point_opacity} r={halfSize} key={id++} fill={props.content_point_color} cx={x + 0.5} cy={y + 0.5} />,
              );
            }
        }
      }
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox={viewBox} {...props}>
      {points}
    </svg>
  );
}

export const qrbtfModuleA1: QrbtfModule<QrbtfRendererA1Props, A1PresetKeys> = {
  type: "svg_renderer",
  presets: A1Presets,
